...
namespace WcfServiceLibrary.ExpensesService
{
    ...
    public class ExpensesService : IExpensesService
    {
        ...
        public void AddExternalExpense(int categoryId, string name, float quantity, float cost, float resultCost, DateTime date, int objectId)
        {
            using (PgSqlConnection conn = new PgSqlConnection(connectionString))
            {
                conn.Open();
                PgSqlCommand pgSqlCommand = new PgSqlCommand("INSERT INTO expenses (category_id,name,quantity," +
                    "cost,result_cost,date,object_id) " +
                    "VALUES (@category_id,@name,@quantity,@cost,@result_cost,@date,@objectId)"
                    , conn);
                pgSqlCommand.Parameters.Add("@category_id", categoryId);
                pgSqlCommand.Parameters.Add("@name", name);
                pgSqlCommand.Parameters.Add("@quantity", quantity);
        ...
        public void EditExternalExpense(int expenseId, int categoryId, string name, float quantity, float cost, float resultCost, DateTime date, int objectId)
        {
            using (PgSqlConnection conn = new PgSqlConnection(connectionString))
            {
                conn.Open();
                PgSqlCommand pgSqlCommand = new PgSqlCommand("UPDATE expenses SET category_id = @categoryId, name = @name," +
                    "quantity = @quantity, cost = @cost, result_cost = @resultCost, date = @date, object_id = @objectId" +
                    " where id = @id", conn);
                pgSqlCommand.Parameters.Add("@id", expenseId);
                pgSqlCommand.Parameters.Add("@categoryId", categoryId);
                pgSqlCommand.Parameters.Add("@name", name);
